...
{
	/// <summary>
	/// Summary description for ScoringModel.
	/// </summary>
	public partial class ScoringModel : System.Web.UI.Page
	{
		...
		private void FillDDL()
		{
			...
			GlobalTools.fillRefList(DDL_TEMPLATE, "SELECT SCOTPL_ID, SCOTPL_DESC FROM PRMSCORING_TEMPLATE", conn);
		}

		private void ViewExistingData()
		{
			conn.QueryString = "SELECT * FROM VW_PRMSCORING_SCORINGMODEL_VIEWEXISTING";
			conn.ExecuteQuery();
		...
		{
			conn.QueryString = "SELECT * FROM VW_PRMSCORING_SCORINGMODEL_VIEWPENDING";
			conn.ExecuteQuery();
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			try
			{
				conn.QueryString = "EXEC PRMSCORING_SCORINGMODEL_MAKER '" + 
		...
		private void DGR_REQUEST_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "delete":
					try
					{
						conn.QueryString = "DELETE FROM PENDING_PRMSCORING_MODEL WHERE PROGRAMID = '"+ e.Item.Cells[0].Text + "' AND SCOTPL_ID = '" + e.Item.Cells[2].Text + "'";
						conn.ExecuteQuery();
					}
					catch (Exception ex)
					{
