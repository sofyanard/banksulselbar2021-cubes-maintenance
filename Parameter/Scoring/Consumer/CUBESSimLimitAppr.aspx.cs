...
{
	/// <summary>
	/// Summary description for CUBESSimLimitAppr.
	/// </summary>
	public partial class CUBESSimLimitAppr : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "select * from VW_PARAM_TEMPLATE_SIM_LIMIT_MAKER_PENDING where SIMTPLID is not null ";
			conn.ExecuteQuery();
		...
		private void performRequestListVal(int row, char appr_sta, string userid)
		{
			...
			{	
				...
				conn.QueryString = "EXEC PARAM_TEMPLATE_SIM_LIMIT_LIST_APPR " + 
					Seq_No + " , '"+ Seq_Id + "' , '" + appr_sta +"', '" +userid+ "'";
		...
		private void ViewPendingDataTpl()
		{
			conn.QueryString="select *,case when CH_STA='0' then 'Update' when CH_STA='1' "+
				"then 'Insert' when CH_STA='2' then 'Delete' else '' end as CH_STA1 "+
				"from MANDIRI_SIM_LIMIT_TTEMPLATE";
			conn.ExecuteQuery();
		...
		private void performRequestTpl(int row)
		{
			try 
			{
				...
				conn.QueryString="select * from MANDIRI_SIM_LIMIT_TTEMPLATE where SIMTPLID='"+TPLID+"'";
				conn.ExecuteQuery();
				string status = conn.GetFieldValue("CH_STA");
				string TPLDESC = conn.GetFieldValue("SIMTPLDESC");

				if (status.Equals("0"))
				{
					conn.QueryString = "EXEC PARAM_TEMPLATE_SIM_LIMIT_APPROVAL '3','"+TPLID+"','" +
				...
				if (status.Equals("1"))
				{
					conn.QueryString = "EXEC PARAM_TEMPLATE_SIM_LIMIT_APPROVAL '4','"+TPLID+"','" +
						TPLDESC+ "', '" +status+"','"+userid+"','"+groupid+"'" ;
					conn.ExecuteNonQuery();
				}
				if (status.Equals("2"))
				{
					conn.QueryString = "EXEC PARAM_TEMPLATE_SIM_LIMIT_APPROVAL '5','"+TPLID+"','" +
						TPLDESC+ "', '" +status+"','"+userid+"','"+groupid+"'" ;
					conn.ExecuteNonQuery();
				}

				conn.QueryString = "EXEC PARAM_TEMPLATE_SIM_LIMIT_APPROVAL '2','"+
					TPLID+"', '', '', '', ''" ;
				conn.ExecuteNonQuery();
