...
{
	/// <summary>
	/// Summary description for CUBESLimitListAppr.
	/// </summary>
	public partial class CUBESLimitListAppr : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "select * from VW_PARAM_LIMITLIST_PENDING";
			conn.ExecuteQuery();
		...
		private void performRequestList(int row, char appr_sta, string userid)
		{
			try 
			{	
				Loan_Code = DGR_APPR.Items[row].Cells[13].Text.Trim();
				Seq_No = DGR_APPR.Items[row].Cells[0].Text.Trim();
				Seq_Id = DGR_APPR.Items[row].Cells[12].Text.Trim();					
