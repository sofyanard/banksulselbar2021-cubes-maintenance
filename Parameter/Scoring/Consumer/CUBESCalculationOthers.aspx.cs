...
{
	/// <summary>
	/// Summary description for CUBESCalculationOthers.
	/// </summary>
	public partial class CUBESCalculationOthers : System.Web.UI.Page
	{
		...
		public void ViewExistingParameterListData()
		{
			...
			{
				DGR_EXISTING_LIST.CurrentPageIndex = DGR_EXISTING_LIST.PageCount - 1;
				DGR_EXISTING_LIST.DataBind();
			}
		...
		public void ViewPendingParameterListData()
		{
			...
			{
				DGR_REQUEST_LIST.CurrentPageIndex = DGR_REQUEST_LIST.PageCount - 1;
				DGR_REQUEST_LIST.DataBind();
			}
		}
		...
		private void ViewExistingParameterValueData()
		{
			if ((this.DDL_PRODUCT_TYPE.SelectedValue  != "") && (this.DDL_PROGRAM_TYPE.SelectedValue != "") && ( this.DDL_EMPLOYEE_TYPE.SelectedValue != "") )
			{
				conn.QueryString = "select PARAM_OTHER_ID, PARAM_OTHER_NAME, PARAM_OTHER_VALUE "+
					"from MANDIRI_PARAM_OTHER "+
				...
				LBL_PARAM_VALUE_ID.Text = conn.GetFieldValue("PARAM_OTHER_ID");
			}
		}
		...
		private void ViewPendingParameterValueData()
		{
			conn.QueryString = "select PARAM_OTHER_ID, PARAM_OTHER_NAME, PARAM_OTHER_VALUE, " +
				...
				"else 'DELETE' end CH_STA1  from TMANDIRI_PARAM_OTHER " +
				"where PARAM_ID = '" + this.Label4.Text.Trim() + "' and REQUEST_ID = '" + LBL_REQUEST.Text.Trim() + "' ";
			conn.ExecuteQuery();
			...
			{
				DGR_REQUEST_VALUE.CurrentPageIndex = DGR_REQUEST_VALUE.CurrentPageIndex - 1;
				DGR_REQUEST_VALUE.DataBind();
			}
		...
		protected void BTN_SAVE_LIST_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				conn.QueryString = "EXEC PARAM_SCORING_MANDIRI_PARAMLIST_OTHER_MAKER '"+ LBL_SAVEMODE.Text.Trim() + "', '" +
					this.TXT_ID1.Text.Trim() + "', '" + TXT_PARAM_NAME.Text.Trim() + "', '" + this.checkApost(TXT_PARAM_FORMULA.Text.Trim()) + "', '" +
					TXT_PARAM_FIELD.Text.Trim() + "', '" + TXT_PARAM_TABLE.Text.Trim() + "', '"+ TXT_PARAM_TABLE_CHILD.Text.Trim() + "', '"+
					this.checkApost(TXT_PARAM_LINK.Text.Trim()) + "', '" + this.RBL_PARAM_PRM.SelectedValue.ToString() + "', '" +
				...
				{
					GlobalTools.popMessage(this, "Error on store procedure!");
				}
		...
		protected void BTN_SAVE_VALUE_Click(object sender, System.EventArgs e)
		{
			if (LBL_SAVEMODE2.Text.Trim() == "1") 
			{
				conn.QueryString = "select * from MANDIRI_PARAM_OTHER " + 
					"WHERE PARAM_OTHER_ID ='" + this.TXT_ID2.Text.Trim() + "' " +
			...
			else
			{
				...
				conn.QueryString = "EXEC PARAM_SCORING_MANDIRI_PARAM_VALUE_OTHER_MAKER '"+ LBL_SAVEMODE2.Text.Trim() +"', '"+
					this.Label4.Text.Trim() +"', '"+ this.LBL_REQUEST.Text.Trim() +"', '"+ this.TXT_ID2.Text.Trim() +"', '"+
					this.TXT_PARAM_NAME2.Text.Trim() +"', '"+ this.checkComma(this.TXT_PARAM_VALUE.Text) +"', '"+
		...
		private void DGR_EXISTING_VALUE_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "edit":
					...
					this.TXT_PARAM_VALUE.Text = cleansText(e.Item.Cells[2].Text);
				...
				case "delete": 
					...
					conn.QueryString = "EXEC PARAM_SCORING_MANDIRI_PARAM_VALUE_OTHER_MAKER '2', '"+ Label4.Text.Trim() + "', '"+
						LBL_REQUEST.Text.Trim() +"', '"+ cleansText(e.Item.Cells[0].Text) +"', '"+
						cleansText(e.Item.Cells[1].Text) +"', '"+ checkComma(cleansFloat(e.Item.Cells[2].Text)) +"', '"+
						seq.ToString() + "'";
					conn.ExecuteNonQuery();
		...
		private void DGR_REQUEST_VALUE_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{
				case "edit":
					LBL_SAVEMODE2.Text = e.Item.Cells[3].Text.Trim();
				...
				case "delete":
					...
					conn.QueryString = "DELETE FROM TMANDIRI_PARAM_OTHER " + 
						...
						"and PARAM_OTHER_ID = '" +cleansText(e.Item.Cells[0].Text)+ "' " +
						"and SEQ_ID = '" + cleansText(e.Item.Cells[4].Text) + "' ";
					conn.ExecuteQuery();
