...
{
	/// <summary>
	/// Summary description for CUBESSimLimit_aspx.
	/// </summary>
	public partial class CUBESSimLimit_aspx : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "select * from VW_PARAM_TEMPLATE_SIM_LIMIT_MAKER_EXSISTING ";
			
			if (this.DDL_TPLID.SelectedValue != "")
				conn.QueryString += "where SIMTPLID = '" + this.DDL_TPLID.SelectedValue + "' ";
		...
		{
			conn.QueryString = "select * from VW_PARAM_TEMPLATE_SIM_LIMIT_MAKER_PENDING ";
		...
		{
			conn.QueryString = "SELECT * FROM MANDIRI_SIM_LIMIT_TEMPLATE";
			conn.ExecuteQuery();
		...
		private void ViewPendingDataTpl()
		{
			...
			conn.QueryString = "select * from MANDIRI_SIM_LIMIT_TTEMPLATE ";
			conn.ExecuteQuery();
		...
		protected void BTN_SAVE_VAL_Click(object sender, System.EventArgs e)
		{
			if (this.LBL_SAVEMODE_VAL.Text == "1" && this.LBL_SEQ_NO.Text == "") //bila insert data baru
			{
				conn.QueryString = "select max(seq_no) + 1 "+
					...
					"	from mandiri_sim_limit_list "+
					"	union "+
					"	select isnull(max(convert(int,seq_no)),0) seq_no "+
					"	from tmandiri_sim_limit_list "+
					"	) alltbl ";
				conn.ExecuteQuery();
			...
			conn.QueryString = "EXEC PARAM_TEMPLATE_SIM_LIMIT_LIST_MAKER '"+ LBL_SAVEMODE_VAL.Text.Trim() + "', "+
				...
				this.DDL_TPLID.SelectedValue +"', '"+ this.LBL_SEQ_ID.Text.Trim() + "'";
			try
			{
		...
		private void DGR_EXISTING_VAL_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "delete": 
					...
					conn.QueryString = "EXEC PARAM_TEMPLATE_SIM_LIMIT_LIST_MAKER '2', "+ 
						...
						cleansText(e.Item.Cells[0].Text) +"', '"+ this.LBL_SEQ_ID.Text.Trim() + "'";
		...
		private void DGR_REQUEST_VAL_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "delete":
					...
					conn.QueryString = "DELETE FROM TMANDIRI_SIM_LIMIT_LIST WHERE SEQ_ID = '"+ seq_id+ "' " +
						"and SEQ_NO = '" + seq_no + "'" ;
					conn.ExecuteQuery();			
					ViewPendingDataVal();
					break;
		...
		{
			conn.QueryString = "EXEC PARAM_TEMPLATE_SIM_LIMIT_MAKER '"+TXT_TPLID.Text+"', '"+TXT_TPLDESC.Text+"'" +
		...
		private void DGR_EXISTING_TPL_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "Edit":
					...
					conn.QueryString="SELECT * FROM MANDIRI_SIM_LIMIT_TEMPLATE WHERE SIMTPLID='"+e.Item.Cells[0].Text.Trim()+"'";
					conn.ExecuteQuery();
					
					TXT_TPLDESC.Text = conn.GetFieldValue("SIMTPLDESC");
					LBL_SAVEMODE_TPL.Text = "0";
										
					break;
				case "Delete":
					conn.QueryString="SELECT * FROM MANDIRI_SIM_LIMIT_TTEMPLATE WHERE SIMTPLID='"+e.Item.Cells[0].Text.Trim()+"'";
					conn.ExecuteQuery();
					if(conn.GetRowCount()!=0)
					{
						conn.QueryString="UPDATE MANDIRI_SIM_LIMIT_TTEMPLATE SET CH_STA='2' WHERE SIMTPLID='"+e.Item.Cells[0].Text.Trim()+"'";
						conn.ExecuteQuery();
					}
					else
					{
						conn.QueryString = "EXEC PARAM_TEMPLATE_SIM_LIMIT_MAKER '" +
		...
		private void DGR_REQUEST_TPL_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				case "Edit":
					...
					{
						TXT_TPLID.Enabled = false;
						TXT_TPLID.Text = conn.GetFieldValue("SIMTPLID");
						TXT_TPLDESC.Text = conn.GetFieldValue("SIMTPLDESC");
				...
				case "Delete":
					...
					conn.QueryString = "DELETE FROM MANDIRI_SIM_LIMIT_TTEMPLATE WHERE SIMTPLID = '"+CODE+"'";
					conn.ExecuteQuery();
					ViewPendingDataTpl();
					filltemplateddl();
					break;
