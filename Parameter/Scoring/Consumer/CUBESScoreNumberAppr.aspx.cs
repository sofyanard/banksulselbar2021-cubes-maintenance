...
{
	/// <summary>
	/// Summary description for CUBESScoreNumberAppr.
	/// </summary>
	public partial class CUBESScoreNumberAppr : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "select * from VW_SCORENUMBER_MAKER_PENDING";
			conn.ExecuteQuery();
		...
		private void performRequestList(int row, char appr_sta, string userid)
		{
			...
			{	
				...
				string PRODUCTID = DGR_APPR.Items[row].Cells[4].Text.Trim();
				
				//conn.QueryString = "EXEC PARAM_SCORING_SCORE_NUMBER_APPR '" + 
				//	SEQ +"' , '"+ appr_sta +"'";

				//Audittrail
				conn.QueryString = "EXEC PARAM_SCORING_SCORE_NUMBER_APPR '" + 
					SEQ +"', '"+ PRODUCTID +"', '"+ appr_sta +"', '" + userid + "'";
				//***
