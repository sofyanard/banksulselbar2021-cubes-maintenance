...
{
	/// <summary>
	/// Summary description for MRAgencyParam.
	/// </summary>
	public partial class MRAgencyParam : System.Web.UI.Page
	{
		...
		private void BindData1()
		{
			conn.QueryString = "select AGENCYID, AGENCYTYPE, AGENCYNAME, CITY_ID, "+ 
				...
				"from TAGENCY where active='1' and AGENCYTYPE = '7' "+ 
		...
		private void BindData2()
		{
			conn.QueryString = "select AGENCYID, AGENCYTYPE, AGENCYNAME, CITY_ID, "+
				...
				"when '3' then 'DELETE' end from TTAGENCY where AGENCYTYPE = '7'";
			conn.ExecuteQuery();
		...
		private void seq(string mode)
		{
			...
			conn.QueryString = "select isnull(max(AGENCYID),0)+ "+LBL_NB.Text+" as MAXSEQ from TAGENCY where AGENCYTYPE = '7' and CITY_ID = '"+DDL_CITY.SelectedValue+"'";
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			conn.QueryString = "SELECT * FROM TTAGENCY WHERE AGENCYID = '"+TXT_AGENCY_CODE.Text+"' AND AGENCYTYPE = '7' AND CITY_ID = '"+DDL_CITY.SelectedValue+"'";
			conn.ExecuteQuery();
			...
			if((hit != 0) && (LBL_SAVE.Text != "1"))
			{
				conn.QueryString = "UPDATE TTAGENCY SET AGENCYNAME = "+GlobalTools.ConvertNull(TXT_AGENCY_NAME.Text)+", "+
					...
					"WHERE AGENCYID = '"+TXT_AGENCY_CODE.Text+"' AND AGENCYTYPE = '7' AND CITY_ID = '"+DDL_CITY.SelectedValue+"'";
			...
			else if((hit == 0) && (LBL_SAVE.Text == "2"))
			{
				conn.QueryString = "insert into TTAGENCY(AGENCYID,AGENCYTYPE,AGENCYNAME,CITY_ID,ADDRESS1,ADDRESS2,ADDRESS3,CITY,ZIPCODE, "+                                                                                                
					...
					" values ('"+TXT_AGENCY_CODE.Text+"','7',"+GlobalTools.ConvertNull(TXT_AGENCY_NAME.Text)+",'"+DDL_CITY.SelectedValue+"',"+
