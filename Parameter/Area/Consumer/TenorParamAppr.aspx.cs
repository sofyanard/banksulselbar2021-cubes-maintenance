...
{
	/// <summary>
	/// Summary description for TenorParamAppr.
	/// </summary>
	public partial class TenorParamAppr : System.Web.UI.Page
	{
		...
		private void performRequest(int row, string userid)
		{
			...
			string pid = DG_APPR.Items[row].Cells[1].Text.Trim();
			string tnseq = DG_APPR.Items[row].Cells[2].Text.Trim();
			string prcode = DG_APPR.Items[row].Cells[3].Text.Trim();
			string tncode = DG_APPR.Items[row].Cells[4].Text.Trim();
			string desc = cleansText(DG_APPR.Items[row].Cells[5].Text);
			string admin = cleansFloat(DG_APPR.Items[row].Cells[6].Text);
			...
			conn.QueryString = "EXEC PARAM_AREA_TENOR_AUDIT '"+pid+"',"+tnseq+",'"+prcode+"','"+status+"','"+userid+"',"+GlobalTools.ConvertNull(tncode)+","+
				GlobalTools.ConvertNull(desc)+","+GlobalTools.ConvertFloat(admin);
			...
			if (status.Equals("1"))
			{
				try
				{
					conn.QueryString = "INSERT INTO RFCAWTENOR VALUES('"+pid+"', "+tnseq+", '"+prcode+"', '"+tncode+
						"', '"+desc+"', "+GlobalTools.ConvertFloat(admin)+",'1')";
			...
			if (status.Equals("2"))
			{
				try
				{
					conn.QueryString = "UPDATE RFCAWTENOR SET TN_CODE = '"+tncode+"', TN_DESC = "+GlobalTools.ConvertNull(desc)+", "+
						"ADMIN_FEE = "+GlobalTools.ConvertFloat(admin)+
						"WHERE PRODUCTID = '"+pid+"' AND TN_SEQ = "+tnseq+" "+
						"AND PR_CODE = '"+prcode+"'";
					conn.ExecuteQuery();
				}
				catch(Exception Ex)
			...
			if (status.Equals("3"))
			{
				try
				{
					conn.QueryString = "UPDATE RFCAWTENOR SET ACTIVE='0' WHERE PRODUCTID = '"+pid+"' AND TN_SEQ = "+tnseq+" "+
								"AND PR_CODE = '"+prcode+"'";
					conn.ExecuteQuery();
				}
				catch(Exception Ex)
