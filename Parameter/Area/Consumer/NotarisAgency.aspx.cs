...
{
	/// <summary>
	/// Summary description for NotarisAgency.
	/// </summary>
	public partial class NotarisAgency : System.Web.UI.Page
	{
		...
		private void BindData1()
		{
			conn.QueryString = "select AGENCYID, AGENCYTYPE, AGENCYNAME, CITY_ID, "+ 
				...
				"from TAGENCY where active='1' and AGENCYTYPE = '4' "+ 
		...
		private void BindData2()
		{
			conn.QueryString = "select AGENCYID, AGENCYTYPE, AGENCYNAME, CITY_ID, "+
				...
				"when '3' then 'DELETE' end from TTAGENCY where AGENCYTYPE = '4'";
			conn.ExecuteQuery();
		...
		private void seq(string mode)
		{
			...
			conn.QueryString = "select isnull(max(AGENCYID),0)+ "+LBL_NB.Text+" as MAXSEQ from TAGENCY where AGENCYTYPE = '4' and CITY_ID = '"+DDL_CITY.SelectedValue+"'";
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			conn.QueryString = "SELECT * FROM TTAGENCY WHERE AGENCYID = '"+TXT_AGENCY_CODE.Text+"' AND AGENCYTYPE = '4' AND CITY_ID = '"+DDL_CITY.SelectedValue+"'";
			conn.ExecuteQuery();
			...
			if((hit != 0) && (LBL_SAVE.Text != "1"))
			{
				conn.QueryString = "UPDATE TTAGENCY SET AGENCYNAME = "+GlobalTools.ConvertNull(TXT_AGENCY_NAME.Text)+", "+
					...
					"CD_SIBS = "+GlobalTools.ConvertNull(TXT_CD_SIBS.Text)+" "+
					"WHERE AGENCYID = '"+TXT_AGENCY_CODE.Text+"' AND AGENCYTYPE = '4' AND CITY_ID = '"+DDL_CITY.SelectedValue+"'";
			...
			else if((hit == 0) && (LBL_SAVE.Text == "2"))
			{
				conn.QueryString = "insert into TTAGENCY(AGENCYID,AGENCYTYPE,AGENCYNAME,CITY_ID,ADDRESS1,ADDRESS2,ADDRESS3,CITY,ZIPCODE, "+                                                                                                
					"EMAIL,CONTACTPERSON,CONTACTHP,PHONEAREA,PHONE,PHONEEXT,FAXAREA,FAXNO,AG_SRCCODE,CD_SIBS,CH_STA) "+
					" values ('"+TXT_AGENCY_CODE.Text+"','4',"+GlobalTools.ConvertNull(TXT_AGENCY_NAME.Text)+",'"+DDL_CITY.SelectedValue+"',"+
					...
					" "+GlobalTools.ConvertNull(TXT_MKT_SOURCE_CODE.Text)+","+GlobalTools.ConvertNull(TXT_CD_SIBS.Text)+", 2)";
			...
			else if((hit == 0) && (LBL_SAVE.Text == "1"))
			{
				conn.QueryString = "insert into TTAGENCY(AGENCYID,AGENCYTYPE,AGENCYNAME,CITY_ID,ADDRESS1,ADDRESS2,ADDRESS3,CITY,ZIPCODE, "+                                                                                                
					...
					" "+GlobalTools.ConvertNull(TXT_PN2.Text)+","+GlobalTools.ConvertNull(TXT_PN3.Text)+","+
					" "+GlobalTools.ConvertNull(TXT_FN1.Text)+","+GlobalTools.ConvertNull(TXT_FN2.Text)+","+
					" "+GlobalTools.ConvertNull(TXT_MKT_SOURCE_CODE.Text)+","+GlobalTools.ConvertNull(TXT_CD_SIBS.Text)+", 1)";
			...
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
						...
						if(conn.GetRowCount() != 0) 
						{
							...
							try
							{
								conn.QueryString = "insert into TTAGENCY(AGENCYID,AGENCYTYPE,AGENCYNAME,CITY_ID,ADDRESS1,ADDRESS2,ADDRESS3,CITY,ZIPCODE,"+                                                                                                
									...
									" "+GlobalTools.ConvertNull(mktsrc)+","+GlobalTools.ConvertNull(cdsibs)+", 3)";
				...
				case "edit":
					...
					if(conn.GetRowCount() != 0) 
					{
						...
						TXT_CD_SIBS.Text = conn.GetFieldValue(0,"CD_SIBS"); 	
					}
