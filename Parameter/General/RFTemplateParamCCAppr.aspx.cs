...
{
	/// <summary>
	/// Summary description for RFTemplateParamCCAppr.
	/// </summary>
	public partial class RFTemplateParamCCAppr : System.Web.UI.Page
	{
		...
		private void performRequest(int row)
		{
			try 
			{
				...
				/*
				conn2.QueryString = "PARAM_GENERAL_CC_" + Request.QueryString["tablename"] + "_APPR " + id + ", '1'";
				conn2.ExecuteQuery();
				*/
		...
		private void deleteData(int row)
		{
			try 
			{
				string id = DGRequest.Items[row].Cells[0].Text.Trim();
				/*
				conn2.QueryString = "PARAM_GENERAL_CC_" + Request.QueryString["tablename"] + "_APPR " + id + ", '0'";
				conn2.ExecuteQuery();
				*/
					...
					else //insert
					{
						if(LBL_ACTIVE.Text.Trim() == "1")
							query = "INSERT INTO " +Request.QueryString["tablename"]+
									"(" + getColumnKey() +"," + getColumnDesc() + ",ACTIVE) VALUES ('"+
									id + "', '" + desc + "', '1')";
						else
							query = "INSERT INTO " +Request.QueryString["tablename"]+
