...
{
	/// <summary>
	/// Summary description for RegionParamAppr.
	/// </summary>
	public partial class RegionParamAppr : System.Web.UI.Page
	{
		...
		private void ViewPendingData()
		{
			...
			{
				this.DGR_APPR.Items[i].Cells[2].Text = GlobalTools.MoneyFormat(this.DGR_APPR.Items[i].Cells[2].Text);
		...
		private void PerformRequest(int row)
		{
			try 
			{
				...
				string region_id		 = DGR_APPR.Items[row].Cells[0].Text.Trim();
				conn2.QueryString = "exec PARAM_CC_RFREGION_APPR '"+ region_id+"','" +
					userid+ "','" + paramname + "','" + paramclass + "'";
				conn2.ExecuteNonQuery();
				//Label1.Text	+= conn2.QueryString;
			} 
			catch {}
		}
		...
		private void DeleteData(int row)
		{
			try 
			{
				string region_id = this.DGR_APPR.Items[row].Cells[0].Text.Trim();
				conn2.QueryString = "delete from PENDING_CC_RFREGION where REGION_ID='" + region_id + "'";
				conn2.ExecuteNonQuery();				
			} 
			catch {}
		}
