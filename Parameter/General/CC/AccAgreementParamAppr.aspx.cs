...
{
	/// <summary>
	/// Summary description for AccAgreementParamAppr.
	/// </summary>
	public partial class AccAgreementParamAppr : System.Web.UI.Page
	{
		...
		private void ViewPendingData()
		{
			...
			{
				this.DGR_APPR.Items[i].Cells[6].Text = getPendingStatus(this.DGR_APPR.Items[i].Cells[6].Text);
				this.DGR_APPR.Items[i].Cells[3].Text = GlobalTools.MoneyFormat(this.DGR_APPR.Items[i].Cells[3].Text);
				this.DGR_APPR.Items[i].Cells[4].Text = GlobalTools.MoneyFormat(this.DGR_APPR.Items[i].Cells[4].Text);
			}			
		}
		private void performRequestList(int row, char appr_sta)
		{
			...
			conn2.QueryString = " exec PARAM_CC_RFACCAGGREMENT_APPR '" + appr_sta + 
				"','" + id + "','" + userid + "','" + paramname + "','" + paramclass+ "'";
