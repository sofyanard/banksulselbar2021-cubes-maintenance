...
{
	/// <summary>
	/// Summary description for SumbanganParamAppr.
	/// </summary>
	public partial class SumbanganParamAppr : System.Web.UI.Page
	{
		...
		private void PerformRequest(int row)
		{
			try 
			{
				...
				string pending_status,id,desc;
				id = DGR_APPR.Items[row].Cells[0].Text.Trim();
				desc = DGR_APPR.Items[row].Cells[1].Text.Trim();
				pending_status = DGR_APPR.Items[row].Cells[2].Text.Trim();
				conn2.QueryString = "exec PARAM_CC_RFSUMBANGANBLN_APPR '" + pending_status + "','"+ id+"','" +
					GlobalTools.ConvertFloat(desc)+ "','" + userid+ "','" + paramname + "','" + paramclass + "'";
				conn2.ExecuteQuery();
			} 
			catch {}
		}
		...
		private void DeleteData(int row)
		{
			try 
			{
				string id = this.DGR_APPR.Items[row].Cells[0].Text.Trim();
