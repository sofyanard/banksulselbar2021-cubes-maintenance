...
{
	/// <summary>
	/// Summary description for GradeParam.
	/// </summary>
	public partial class GradeParam : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				GlobalTools.fillRefList(DDL_CATEGORY_ID,"select CATEGORY_ID,CATEGORY_ID + ' - ' + CATEGORY_DESC AS CATEGORY_DESC from rfcustcategory where ACTIVE='1'",conn2);
				ViewExistingData();
			}
			ViewPendingData();
		...
		{
			this.DDL_CATEGORY_ID.SelectedValue	= "";
			this.TXT_GOL_ID.Text				= "";
			this.TXT_GOL_DESC.Text				= "";
			this.TXT_GOL_SHORTDESC.Text			= "";
			this.activateControlKey(true);
		}

		private void activateControlKey(bool isReadOnly) 
		{
			this.DDL_CATEGORY_ID.Enabled = isReadOnly;
			this.TXT_GOL_ID.ReadOnly	 = !isReadOnly;
		}
		...
		{
			conn2.QueryString = "SELECT * FROM VW_PARAM_CC_RFJOB_TYPE_GOLONGAN";
			conn2.ExecuteQuery();
		...
		{
			conn2.QueryString = "SELECT * FROM VW_PARAM_PENDING_CC_RFJOB_TYPE_GOLONGAN ";
			conn2.ExecuteQuery();
			DataTable dt = new DataTable();
			dt = conn2.GetDataTable().Copy();
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			{
				conn2.QueryString = "select * from VW_PARAM_CC_RFJOB_TYPE_GOLONGAN where CATEGORY_ID ='"+ 
					this.DDL_CATEGORY_ID.SelectedValue.Trim() + "' and GOL_ID='" + this.TXT_GOL_ID.Text.Trim() + "'";
				conn2.ExecuteQuery();
				int jml1 = conn2.GetRowCount();
				if (jml1 > 0) 
				{
					GlobalTools.popMessage(this, "ID has already been used! Request canceled!");
					GlobalTools.SetFocus(this,DDL_CATEGORY_ID);
					return;
				}
			}		
		
			conn2.QueryString = "exec PARAM_GENERAL_CC_RFJOB_TYPE_GOLONGAN_MAKER  '"+this.LBL_SAVEMODE.Text +"','" +
				this.DDL_CATEGORY_ID.SelectedValue + "','" + this.TXT_GOL_ID.Text.Trim() + "','" +
				this.TXT_GOL_DESC.Text.Trim() + "','" + this.TXT_GOL_SHORTDESC.Text.Trim() + "'";
			conn2.ExecuteQuery();
		...
		private void DGR_EXISTING_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				case "edit":
					LBL_SAVEMODE.Text = "0";
					this.DDL_CATEGORY_ID.SelectedValue	= CleansText(e.Item.Cells[0].Text.Trim());
					this.TXT_GOL_ID.Text				= CleansText(e.Item.Cells[2].Text.Trim());
					this.TXT_GOL_DESC.Text				= CleansText(e.Item.Cells[3].Text.Trim());
					this.TXT_GOL_SHORTDESC.Text			= CleansText(e.Item.Cells[4].Text.Trim());
					activateControlKey(false);
					break;
				case "delete":
					conn2.QueryString = "exec PARAM_GENERAL_CC_RFJOB_TYPE_GOLONGAN_MAKER  '2','" +
		...
		private void DGR_REQUEST_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				case "edit":
					...
					break;
				case "delete":
					string category_id	= e.Item.Cells[0].Text.Trim();
					string gol_id		= e.Item.Cells[2].Text.Trim();
					conn2.QueryString = "delete from PENDING_CC_RFJOB_TYPE_GOLONGAN WHERE CATEGORY_ID='" + 
						category_id + "' and GOL_ID ='" + gol_id + "'";
