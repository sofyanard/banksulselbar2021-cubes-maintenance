...
{
	/// <summary>
	/// Summary description for AgencyParamAppr.
	/// 
	/// </summary>
	public partial class AgencyParamAppr : System.Web.UI.Page
	{
		...
		{
			conn2.QueryString = "select * from PENDING_CC_TAGENCY WHERE ACTIVE = '1'";
			conn2.ExecuteQuery();
			...
			{
				this.DGR_APPR.Items[i].Cells[9].Text = this.getPendingStatus(this.DGR_APPR.Items[i].Cells[9].Text);
			}
		}	
		...
		private void performRequestList(int row, char appr_sta)
		{
			...
			string agencyid = this.DGR_APPR.Items[row].Cells[0].Text.Trim();
			string paramid	= "104";
			...
			conn2.QueryString = " exec PARAM_CC_TAGENCY_APPR '" + 
				appr_sta + "','" + agencyid + "','" + userid + "','" + paramname + "','" + paramclass+ "'";
			conn2.ExecuteQuery();
		}
