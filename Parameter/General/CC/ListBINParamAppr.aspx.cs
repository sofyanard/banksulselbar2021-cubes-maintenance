...
{
	/// <summary>
	/// Summary description for ListBINParamAppr.
	/// </summary>
	public partial class ListBINParamAppr : System.Web.UI.Page
	{
		...
		private void ViewPendingData()
		{
			...
			{
				this.DGR_APPR.Items[i].Cells[6].Text = getPendingStatus(DGR_APPR.Items[i].Cells[6].Text) ;
			}			
		}
		...
		private void PerformRequest(int row)
		{
			try 
			{
				...
				string pending_status,binseq,bn_code,bin_number,ica;
				binseq = DGR_APPR.Items[row].Cells[0].Text.Trim();
				bn_code = DGR_APPR.Items[row].Cells[1].Text.Trim();
				bin_number = DGR_APPR.Items[row].Cells[3].Text.Trim();
				ica = DGR_APPR.Items[row].Cells[4].Text.Trim();
				pending_status = DGR_APPR.Items[row].Cells[5].Text.Trim();
				conn2.QueryString = "exec PARAM_CC_CCBIN_APPR '" + pending_status + "',"+ binseq+",'" +
					bn_code+"','" +bin_number+ "','" + ica + "','" + userid + "'";
				conn2.ExecuteQuery();
		...
		private void DeleteData(int row)
		{
			try 
			{
				string binseq = this.DGR_APPR.Items[row].Cells[0].Text.Trim();
				string bn_code = this.DGR_APPR.Items[row].Cells[1].Text.Trim();
				conn2.QueryString = "delete from PENDING_CC_CCBIN where BINSEQ='" + binseq + "' " +
					"and BN_CODE='" + bn_code + "'";
				conn2.ExecuteQuery();				
			} 
			catch {}
