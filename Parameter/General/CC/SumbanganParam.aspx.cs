...
{
	/// <summary>
	/// Summary description for SumbanganParam.
	/// </summary>
	public partial class SumbanganParam : System.Web.UI.Page
	{
		...
		{
			this.TXT_ID.ReadOnly = isReadOnly;
		}

		private void ClearBoxes()
		{
			this.TXT_ID.Text			= "";
			this.TXT_SUMBANGAN.Text		= "";
		...
		{
			conn2.QueryString = "SELECT * FROM RFSUMBANGANBLN where ACTIVE ='1'";
			conn2.ExecuteQuery();
			...
			{
				this.DGR_EXISTING.Items[i].Cells[1].Text = GlobalTools.MoneyFormat(this.DGR_EXISTING.Items[i].Cells[1].Text);
			}		
		}

		private void ViewPendingData()
		{
			conn2.QueryString = "select * from PENDING_CC_RFSUMBANGANBLN";
			conn2.ExecuteQuery();
			DataTable dt = new DataTable();
			dt = conn2.GetDataTable().Copy();
			...
			{
				this.DGR_REQUEST.Items[i].Cells[1].Text = GlobalTools.MoneyFormat(this.DGR_REQUEST.Items[i].Cells[1].Text);
				this.DGR_REQUEST.Items[i].Cells[3].Text = getPendingStatus(this.DGR_REQUEST.Items[i].Cells[3].Text);
			}			
		}
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			{
				conn2.QueryString = "select * from RFSUMBANGANBLN where ACTIVE ='1' and ID_SUMBANG_BLN ='"+ this.TXT_ID.Text +"'";
				conn2.ExecuteQuery();
				int jml1 = conn2.GetRowCount();
				/*
				conn2.QueryString = "select * from PENDING_CC_RFSUMBANGANBLN where ID_SUMBANG_BLN ='"+ this.TXT_ID.Text +"'";
				conn2.ExecuteQuery();
			...
			conn2.QueryString = "exec PARAM_GENERAL_CC_RFSUMBANGANBLN_MAKER '"+this.LBL_SAVEMODE.Text +"','" +
				this.TXT_ID.Text + "','" + GlobalTools.ConvertFloat(this.TXT_SUMBANGAN.Text) + "'";
			conn2.ExecuteQuery();
		...
		private void DGR_EXISTING_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				case "edit":
					LBL_SAVEMODE.Text = "0";
					this.TXT_ID.Text			= CleansText(e.Item.Cells[0].Text);
					this.TXT_SUMBANGAN.Text		= CleansText(e.Item.Cells[1].Text);
					activateControlKey(true);
					break;
				case "delete":	
					conn2.QueryString = "exec PARAM_GENERAL_CC_RFSUMBANGANBLN_MAKER '2','" + 
						CleansText(e.Item.Cells[0].Text) + "','" +
						GlobalTools.ConvertFloat(CleansText(e.Item.Cells[1].Text)) + "'";
		...
		private void DGR_REQUEST_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				...
				case "delete":
					...
					conn2.QueryString = "delete from PENDING_CC_RFSUMBANGANBLN WHERE ID_SUMBANG_BLN='" + id + "'";
					conn2.ExecuteQuery();
					ViewPendingData();
					break;
				default :
