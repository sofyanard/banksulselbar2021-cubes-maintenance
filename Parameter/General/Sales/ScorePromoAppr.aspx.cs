...
{
	/// <summary>
	/// Summary description for ScorePromoAppr.
	/// </summary>
	public partial class ScorePromoAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string id = DGR_APPR.Items[row].Cells[2].Text.Trim();
				string SEQ_ID = DGR_APPR.Items[row].Cells[12].Text.Trim();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_SCORE_PROMOTION where ID='"+id+"' and "+
					"SEQ_ID='"+SEQ_ID+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string AGENT_TYPE =conncc.GetFieldValue("AGENT_TYPE");
				string MIN_PER_TARGET =conncc.GetFieldValue("MIN_PER_TARGET");
				string MAX_PER_TARGET =conncc.GetFieldValue("MAX_PER_TARGET");
				string VALUE_FINAL =conncc.GetFieldValue("VALUE_FINAL");
				string DECISION =conncc.GetFieldValue("DECISION");
				string ACTIVE =conncc.GetFieldValue("ACTIVE");
				
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_SCOREPROMO_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					id +"','"+ AGENT_TYPE +"','"+MIN_PER_TARGET+"','"+MAX_PER_TARGET+"','"+
					VALUE_FINAL +"','"+DECISION+"','"+ACTIVE +"','"+SEQ_ID+"','"+userid +"','"+groupid+"'";
				conncc.ExecuteQuery();
			}
			catch{}
		}
