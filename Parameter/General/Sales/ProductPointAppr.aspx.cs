...
{
	/// <summary>
	/// Summary description for ProductPointAppr.
	/// </summary>
	public partial class ProductPointAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string Prod_ID = DGR_APPR.Items[row].Cells[0].Text.Trim();
				string SEQ_ID = DGR_APPR.Items[row].Cells[8].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_PRODUCT_POINT where PRODUCT_ID='"+Prod_ID+"' and "+
					" SEQ_ID='"+SEQ_ID+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string point = conncc.GetFieldValue("PRODUCT_POINT");
				string reimburse = conncc.GetFieldValue("Product_Reimbursment");
				string active = conncc.GetFieldValue("ACTIVE");

				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_PRODUCT_POINT_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					Prod_ID +"','"+ point +"','"+ reimburse +"','"+ active +"','"+SEQ_ID+"','"+
					userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
