...
{
	/// <summary>
	/// Summary description for AwardTypeAppr.
	/// </summary>
	public partial class AwardTypeAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string award_id = DGR_APPR.Items[row].Cells[0].Text.Trim();
				string seq_id = DGR_APPR.Items[row].Cells[6].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_AWARDTYPE where AWARD_ID='"+award_id+"' and "+
					"SEQ_ID ='"+seq_id+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string AWARD_DESC =conncc.GetFieldValue("AWARD_DESC");
				string ACTIVE =conncc.GetFieldValue("ACTIVE");
						
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_AWARDTYPE_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					award_id +"','"+ AWARD_DESC +"','"+ ACTIVE +"','"+seq_id+"','"+userid+"','"+groupid+"'";
				conncc.ExecuteQuery();
			}
			catch {}
