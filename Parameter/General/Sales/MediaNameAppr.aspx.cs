...
{
	/// <summary>
	/// Summary description for MediaNameAppr.
	/// </summary>
	public partial class MediaNameAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string MN_ID = DGR_APPR.Items[row].Cells[1].Text.Trim();
				string MN_DESC = DGR_APPR.Items[row].Cells[2].Text.Trim();
				string MD_ID = DGR_APPR.Items[row].Cells[7].Text.Trim();
				string SEQ_ID = DGR_APPR.Items[row].Cells[8].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_RFMEDIANAME where MN_ID='"+MN_ID+"' and "+
					"MN_DESC='"+MN_DESC+"' and MD_ID='"+MD_ID+"' and SEQ_ID='"+SEQ_ID+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string ACTIVE =conncc.GetFieldValue("ACTIVE");
						
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_RFMEDIANAME_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					MN_ID +"','"+ MN_DESC +"','"+MD_ID+"','"+ACTIVE +"','"+SEQ_ID+"','"+
					userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
