...
{
	/// <summary>
	/// Summary description for AdmFeeAppr.
	/// </summary>
	public partial class AdmFeeAppr : System.Web.UI.Page
	{
		...
		protected Label lbl_a = new Label(); 
		...
		private void ViewPendingData()
		{
			...
			conncc.QueryString="select Min_Val, Max_Val, Currency, _Value, AF_CODE, a.Area_ID, AREA_NAME, SEQ_ID "+
				",case when STATUS = '1' then 'Insert' when STATUS = '0' then 'Update' "+
				"else 'Delete' end STATUS1 "+
				"from PENDING_SALESCOM_ADMFEE a "+
				"left join "+ this.LBL_DB_NAMA.Text +".dbo.AREA b on a.AREA_ID=b.AREA_ID ";
			conncc.ExecuteQuery();
		...
		private void performRequestList(int row, int appr_sta)
		{
			try
			{
				...
				lbl_a.ID = "lbl_a";
				lbl_a.Text = "aaa";
				string seq_id = DGR_APPR.Items[row].Cells[1].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_ADMFEE where seq_id='"+seq_id+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				float Min_Val =float.Parse(conncc.GetFieldValue("Min_Val"));
				float Max_Val =float.Parse(conncc.GetFieldValue("Max_Val"));
				string Currency =conncc.GetFieldValue("Currency");
				float Valuenya =float.Parse(conncc.GetFieldValue("_Value"));
				string Area_ID =conncc.GetFieldValue("Area_ID");
				string AF_CODE =conncc.GetFieldValue("AF_CODE");
				string ACTIVE =conncc.GetFieldValue("ACTIVE");
					
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_ADMFEE_APPR '" + 
					seq_id + "' , '"+ appr_sta + "' , 'LOSSALES','"+ status +"','"+
					Min_Val +"','"+ Max_Val +"','"+ Currency +"','"+ Valuenya +"','"+
					Area_ID +"','"+ AF_CODE +"','"+ ACTIVE +"','"+userid+"','"+groupid+"'";
				conncc.ExecuteQuery();
			}
			catch{}
		}
