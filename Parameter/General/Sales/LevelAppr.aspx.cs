...
{
	/// <summary>
	/// Summary description for LevelAppr.
	/// </summary>
	public partial class LevelAppr : System.Web.UI.Page
	{
		...
		{
			//SMEDEV2
			conncc.QueryString="select * from VW_PARAM_SALESCOM_PENDING_LEVEL order by LEVEL_CODE";
			conncc.ExecuteQuery();
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string code = DGR_APPR.Items[row].Cells[0].Text.Trim();
				string seq_id = DGR_APPR.Items[row].Cells[7].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_LEVEL where LEVEL_CODE='"+code+"' and "+
					"SEQ_ID='"+seq_id+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string AGENTYPE_ID =conncc.GetFieldValue("AGENTYPE_ID");
				string LEVEL_DESC =conncc.GetFieldValue("LEVEL_DESC");
				string L_LEVEL =conncc.GetFieldValue("L_LEVEL");
				string ACTIVE =conncc.GetFieldValue("ACTIVE");
		
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_LEVEL_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					code +"','"+ AGENTYPE_ID +"','"+LEVEL_DESC+"','"+ 
					L_LEVEL +"','"+ACTIVE +"','"+seq_id+"','"+
					userid +"','"+groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
