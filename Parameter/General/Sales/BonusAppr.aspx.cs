...
{
	/// <summary>
	/// Summary description for BonusAppr.
	/// </summary>
	public partial class BonusAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			try
			{
				...
				string Agentype = DGR_APPR.Items[row].Cells[9].Text.Trim();
				string level = DGR_APPR.Items[row].Cells[10].Text.Trim();
				int seq_no = int.Parse(DGR_APPR.Items[row].Cells[11].Text.Trim());
				int seq_id = int.Parse(DGR_APPR.Items[row].Cells[12].Text.Trim());
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_AGENT_BONUSPOINT where AGENTYPE_ID='"+Agentype+"' and LEVEL_CODE='"+level+"' and "+
					"SEQ_NO ='"+seq_no+"' and SEQ_ID='"+seq_id+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string MIN_POINT1 = GlobalTools.ConvertFloat(conncc.GetFieldValue("MIN_POINT"));
				//float MIN_POINT=float.Parse(MIN_POINT1);
				string MAX_POINT1 = GlobalTools.ConvertFloat(conncc.GetFieldValue("MAX_POINT"));
				//float MAX_POINT = float.Parse(MAX_POINT1);
				string BONUS_VALUE1 = GlobalTools.ConvertFloat(conncc.GetFieldValue("BONUS_VALUE"));
				//int BONUS_VALUE = int.Parse(BONUS_VALUE1);
				string active = conncc.GetFieldValue("ACTIVE");
			
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_AGENT_BONUSPOINT_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					Agentype +"','"+ level +"','"+ seq_no +"','"+
					MIN_POINT1 +"','"+ MAX_POINT1 +"','"+ BONUS_VALUE1 +"','"+ seq_id +"','"+ active +"','"+ userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch (Exception p)
			{
