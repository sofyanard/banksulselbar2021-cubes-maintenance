...
{
	/// <summary>
	/// Summary description for MediaAppr.
	/// </summary>
	public partial class MediaAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string MD_ID = DGR_APPR.Items[row].Cells[1].Text.Trim();
				string MD_DESC = DGR_APPR.Items[row].Cells[2].Text.Trim();
				string SC_ID = DGR_APPR.Items[row].Cells[7].Text.Trim();
				string SEQ_ID = DGR_APPR.Items[row].Cells[8].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_RFMEDIA where MD_ID='"+MD_ID+"' and MD_DESC='"+MD_DESC+"' and "+
					"SC_ID='"+SC_ID+"' and SEQ_ID='"+SEQ_ID+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string ACTIVE =conncc.GetFieldValue("ACTIVE");
				
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_RFMEDIA_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					MD_ID +"','"+ MD_DESC +"','"+SC_ID+"','"+ACTIVE +"','"+SEQ_ID+"','"+
					userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch (Exception p)
