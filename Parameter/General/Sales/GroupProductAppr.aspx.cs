...
{
	/// <summary>
	/// Summary description for GroupProductAppr.
	/// </summary>
	public partial class GroupProductAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string group_id = DGR_APPR.Items[row].Cells[1].Text.Trim();
				string seq_id = DGR_APPR.Items[row].Cells[0].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_RFGROUP where GROUP_ID='"+group_id+"' and SEQ_ID='"+seq_id+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string group_name = conncc.GetFieldValue("GROUP_NAME");
				string active = conncc.GetFieldValue("ACTIVE");
						
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_RFGROUP_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					group_id +"','"+ group_name +"','"+ seq_id +"','"+ active +"','"+
					userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
