...
{
	/// <summary>
	/// Summary description for CalculationTypeAppr.
	/// </summary>
	public partial class CalculationTypeAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			try
			{
				...
				string CAL_ID = DGR_APPR.Items[row].Cells[0].Text.Trim();
				string SEQ_ID = DGR_APPR.Items[row].Cells[11].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_CALCULATION_TYPE where CALCULATION_ID='"+CAL_ID+"' and "+
					"SEQ_ID='"+SEQ_ID+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string desc = conncc.GetFieldValue("CALCULATION_DESC");
				string track = conncc.GetFieldValue("CAL_TRACK");
				string fielddate = conncc.GetFieldValue("CAL_FIELDDATE");
				string fieldtrack = conncc.GetFieldValue("CAL_FIELDTRACK");
				string incentive = conncc.GetFieldValue("CAL_INCENTIVE");
				string loanAmount = conncc.GetFieldValue("CAL_LOANAMOUNT");
				string active = conncc.GetFieldValue("ACTIVE");
			
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_CALCULATION_TYPE_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					CAL_ID +"','"+ desc +"','"+ track +"','"+
					fielddate +"','"+ fieldtrack +"','"+ incentive +"','"+ loanAmount +"','"+ 
					active +"','"+ SEQ_ID +"','"+ userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
		}
