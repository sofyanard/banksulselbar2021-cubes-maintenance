...
namespace CuBES_Maintenance.Parameter.General.Sales
{
	...
	public partial class GroupProduct : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				conncc.QueryString="select isnull(max(GROUP_ID), 0)+1 GROUP_ID from RFGROUP";
				conncc.ExecuteQuery();
				string IDnya=conncc.GetFieldValue("GROUP_ID");
		...
		{
			conncc.QueryString="select * from VW_PARAM_SALESCOM_RFGROUP";
			conncc.ExecuteQuery();
		...
		{
			conncc.QueryString="Select * from VW_PARAM_SALESCOM_PENDING_RFGROUP";
			conncc.ExecuteQuery();
			DataTable dt = new DataTable();
			dt = conncc.GetDataTable().Copy();
		...
		private void DGR_EXISTING_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "delete":
					...
					//Get Seq_id....
					conncc.QueryString="select isnull(max(Seq_ID), 0)+1 SEQ_ID from PENDING_SALESCOM_RFGROUP";
					conncc.ExecuteQuery();
					string seq_id=conncc.GetFieldValue("SEQ_ID");

					conncc.QueryString="select * from PENDING_SALESCOM_RFGROUP "+
						"where GROUP_ID='"+e.Item.Cells[2].Text+"'";
					conncc.ExecuteQuery();
					if(conncc.GetRowCount()!=0)
					{
						conncc.QueryString="update PENDING_SALESCOM_RFGROUP set STATUS='2' where GROUP_ID='"+e.Item.Cells[2].Text+"'";
						conncc.ExecuteQuery();
					}
					else
					{
						conncc.QueryString="PARAM_SALESCOM_RFGROUP_MAKER '2','"+
		...
		private void DGR_REQUEST_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "delete":
					...
					conncc.QueryString="Delete from PENDING_SALESCOM_RFGROUP where GROUP_ID='"+id+"'and Seq_ID='"+seq_id+"'"; 
					try
					{
						conncc.ExecuteQuery();
					}
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			if (LBL_ID.Text == "" && LBL_JENIS.Text=="" && LBL_SEQ_ID.Text=="") //input baru
			{
				...
				string seq_id=conncc.GetFieldValue("SEQ_ID");
				//SalesMandiri
				conncc.QueryString="select * from RFGROUP where GROUP_ID='"+TXT_ID.Text+"'";
				conncc.ExecuteQuery();
				if(conncc.GetRowCount()!=0)
				{
					GlobalTools.popMessage(this,"GROUP ID is Existing..");
					return;
				}
				//SMEDEV2
				conncc.QueryString="select GROUP_ID from PENDING_SALESCOM_RFGROUP ";
				conncc.ExecuteQuery();
				for(int i=0; i<conncc.GetRowCount(); i++)
				{
					if(conncc.GetFieldValue(i,0).Trim()==TXT_ID.Text)
					{
				...
				conncc.QueryString="PARAM_SALESCOM_RFGROUP_MAKER '"+LBL_SAVE_MODE.Text+"','"+ 
					TXT_ID.Text +"','"+ TXT_DESC.Text +"','"+seq_id+"','1'";
				conncc.ExecuteQuery();
				BlankEntry();
				ViewPendingData();
			}
			if (LBL_JENIS.Text=="edit" && LBL_ID.Text!="") //edit
			{
				...
				else if(LBL_SEQ_ID.Text=="")//edet dari Existing
				{
					//cek tabel pending
					conncc.QueryString="select * from PENDING_SALESCOM_RFGROUP where GROUP_ID='"+TXT_ID.Text+"'";
					conncc.ExecuteQuery();
					...
					conncc.QueryString="PARAM_SALESCOM_RFGROUP_MAKER '0','"+ 
						TXT_ID.Text +"','"+ TXT_DESC.Text +"','"+LBL_SEQ_ID.Text+"','1'";
					conncc.ExecuteQuery();
					BlankEntry();
					ViewPendingData();
