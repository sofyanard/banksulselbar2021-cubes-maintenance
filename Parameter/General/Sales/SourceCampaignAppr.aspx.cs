...
{
	/// <summary>
	/// Summary description for SourceCampaignAppr.
	/// </summary>
	public partial class SourceCampaignAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string sc_id = DGR_APPR.Items[row].Cells[1].Text.Trim();
				string seq_id = DGR_APPR.Items[row].Cells[0].Text.Trim();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_RFSOURCE_CAMPAIGN where SC_ID='"+sc_id+"' and SEQ_ID='"+seq_id+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string sc_desc = conncc.GetFieldValue("SC_DESC");
				string active = conncc.GetFieldValue("ACTIVE");
						
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_RFSOURCE_CAMPAIGN_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					sc_id +"','"+ sc_desc +"','"+ seq_id +"','"+ active +"','"+ userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
		}
