...
{
	/// <summary>
	/// Summary description for InitialAppr.
	/// </summary>
	public partial class InitialAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			try 
			{	
				int in_seq = int.Parse(DGR_APPR.Items[row].Cells[0].Text.Trim());
				string userid = Session["UserID"].ToString();
				string groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_INITIAL where IN_SEQ='"+in_seq+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string photo = conncc.GetFieldValue("IN_PHOTODIRECTORY");
				int max_child = int.Parse(conncc.GetFieldValue("IN_MAXTANGGUNGAN"));
				float pph = float.Parse(conncc.GetFieldValue("IN_PPHPS21"));
				string active = conncc.GetFieldValue("ACTIVE");
						
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_INITIAL_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					in_seq +"','"+ photo +"','"+ max_child +"','"+ pph +"','"+ active +"','"+
					userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
