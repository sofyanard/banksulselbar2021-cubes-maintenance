...
{
	/// <summary>
	/// Summary description for CriteriaTypeAppr.
	/// </summary>
	public partial class CriteriaTypeAppr : System.Web.UI.Page
	{
		...
		{
			conncc.QueryString="select * from VW_PARAM_SALESCOM_PENDING_RFCRITERIA_TYPE order by CRITERIA_ID";
			conncc.ExecuteQuery();
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				int seq_id = Convert.ToInt16(DGR_APPR.Items[row].Cells[12].Text.Trim());
				string Criteria_id = this.DGR_APPR.Items[row].Cells[0].Text;
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_RFCRITERIA_TYPE "+
					"where CRITERIA_ID='"+Criteria_id+"' and SEQ_ID='"+ seq_id +"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string CRITERIA_DESC =conncc.GetFieldValue("CRITERIA_DESC");
				string CAL_TRACK =conncc.GetFieldValue("CAL_TRACK");
				string CAL_FIELDDATE =conncc.GetFieldValue("CAL_FIELDDATE");
				string CAL_FIELDTRACK =conncc.GetFieldValue("CAL_FIELDTRACK");
				string CAL_FIELDSELECT =conncc.GetFieldValue("CAL_FIELDSELECT");
				string CAL_FIELDGROUP =conncc.GetFieldValue("CAL_FIELDGROUP");
				string CAL_TYPE =conncc.GetFieldValue("CAL_TYPE");
						
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_RFCRITERIA_TYPE_APPR '" + 
					appr_sta + "','SMEDEV2','"+ status +"','"+
					Criteria_id +"','"+ CRITERIA_DESC +"','"+ 
					CAL_TRACK +"','"+ CAL_FIELDDATE +"','"+ CAL_FIELDTRACK +"','"+
					CAL_FIELDSELECT +"','"+ CAL_FIELDGROUP +"','"+ CAL_TYPE +"','"+
					seq_id +"','1','"+ userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
		}
