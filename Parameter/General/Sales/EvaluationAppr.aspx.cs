...
{
	/// <summary>
	/// Summary description for EvaluationAppr.
	/// </summary>
	public partial class EvaluationAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string eval_id = DGR_APPR.Items[row].Cells[1].Text.Trim();
				string seq_id = DGR_APPR.Items[row].Cells[0].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_RFEvaluation where Evaluation_ID='"+eval_id+"' and Seq_ID='"+seq_id+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string eval_desc = conncc.GetFieldValue("Evaluation_Desc");
				string active = conncc.GetFieldValue("ACTIVE");
						
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_RFEvaluation_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					eval_id +"','"+ eval_desc +"','"+ seq_id +"','"+ active +"','"+
					userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
