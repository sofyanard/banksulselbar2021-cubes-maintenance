...
{
	/// <summary>
	/// Summary description for EvaluationDetailAppr.
	/// </summary>
	public partial class EvaluationDetailAppr : System.Web.UI.Page
	{
		...
		{
			conncc.QueryString="select a.evaluation_typeid, a.evaluation_typests, a.evaluation_id, "+
				"a.evaluation_desc, a.evaluation_max, a.active, "+
				"b.evaluation_desc as description,case when STATUS = '1' then 'Insert' "+
				"when STATUS = '0' then 'Update' else 'Delete' end STATUS1,a.STATUS,a.SEQ_ID "+
				"from pending_salescom_evaluation_type a left join SALESMANDIRI.dbo.rfevaluation b "+
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string type_id = DGR_APPR.Items[row].Cells[7].Text.Trim();
				string SEQ_ID = DGR_APPR.Items[row].Cells[0].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_EVALUATION_TYPE where EVALUATION_TYPEID='"+type_id+"' and "+
					"SEQ_ID = '"+SEQ_ID+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string EVALUATION_TYPESTS =conncc.GetFieldValue("EVALUATION_TYPESTS");
				string EVALUATION_ID =conncc.GetFieldValue("EVALUATION_ID");
				string EVALUATION_DESC =conncc.GetFieldValue("EVALUATION_DESC");
				string EVALUATION_MAX =GlobalTools.ConvertFloat(conncc.GetFieldValue("EVALUATION_MAX"));
				string ACTIVE =conncc.GetFieldValue("ACTIVE");
						
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_EVALUATIONTYPE_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					type_id +"','"+ EVALUATION_TYPESTS +"','"+EVALUATION_ID+"','"+ 
					EVALUATION_DESC +"','"+EVALUATION_MAX+"','"+ACTIVE +"','"+SEQ_ID+"','"+
					userid +"','"+groupid+"'";
				conncc.ExecuteQuery();
			}
			catch{}
