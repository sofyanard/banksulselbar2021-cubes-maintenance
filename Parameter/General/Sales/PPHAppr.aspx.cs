...
{
	/// <summary>
	/// Summary description for PPHAppr.
	/// </summary>
	public partial class PPHAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_RFPERSONAL_PPH where PPH_CODE='"+code+"' and "+
					"SEQ_ID='"+seq_id+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string PPH_MIN =conncc.GetFieldValue("PPH_MIN");
				string PPH_MAX =conncc.GetFieldValue("PPH_MAX");
				string PPH_PERCENT =conncc.GetFieldValue("PPH_PERCENT");
				string ACTIVE =conncc.GetFieldValue("ACTIVE");
			
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_RFPPH_APPR '" + 
					appr_sta + "' , 'SMEDEV2','"+ status +"','"+
					code +"','"+ PPH_MIN +"','"+PPH_MAX+"','"+ 
					PPH_PERCENT +"','"+ACTIVE +"','"+seq_id+"','"+
					userid +"','"+groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
