...
namespace CuBES_Maintenance.Parameter.General.Sales
{
	...
	public partial class Evaluation : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				conncc.QueryString="select isnull(max(Evaluation_ID), 0)+1 Eval_ID from RFEvaluation";
				conncc.ExecuteQuery();
				string IDnya=conncc.GetFieldValue("Eval_ID");
		...
		{
			conncc.QueryString="select * from VW_PARAM_SALESCOM_RFEvaluation";
			conncc.ExecuteQuery();
		...
		{
			conncc.QueryString="Select * from VW_PARAM_SALESCOM_PENDING_RFEvaluation";
			conncc.ExecuteQuery();
			DataTable dt = new DataTable();
			dt = conncc.GetDataTable().Copy();
		...
		private void DGR_EXISTING_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "delete":
					...
					//Get Seq_id....
					conncc.QueryString="select isnull(max(Seq_ID), 0)+1 SEQ_ID from PENDING_SALESCOM_RFEvaluation";
					conncc.ExecuteQuery();
					string seq_id=conncc.GetFieldValue("SEQ_ID");
					
					conncc.QueryString="select * from PENDING_SALESCOM_RFEvaluation "+
						"where Evaluation_ID='"+e.Item.Cells[2].Text+"'";
					conncc.ExecuteQuery();
					if(conncc.GetRowCount()!=0)
					{
						conncc.QueryString="update PENDING_SALESCOM_RFEvaluation set STATUS='2' where Evaluation_ID='"+e.Item.Cells[2].Text+"'";
						conncc.ExecuteQuery();
					}
					else
					{
						conncc.QueryString="PARAM_SALESCOM_RFEvaluation_MAKER '2','"+
		...
		private void DGR_REQUEST_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName)
			{
				...
				case "delete":
					...
					conncc.QueryString="Delete from PENDING_SALESCOM_RFEvaluation where Evaluation_ID='"+id+"'and Seq_ID='"+seq_id+"'"; 
					try
					{
						conncc.ExecuteQuery();
					}
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			if (LBL_ID.Text == "" && LBL_JENIS.Text=="" && LBL_SEQ_ID.Text=="") //input baru
			{
				...
				//SalesMandiri
				conncc.QueryString="select * from RFEvaluation where Evaluation_ID='"+TXT_ID.Text+"'";
				conncc.ExecuteQuery();
				if(conncc.GetRowCount()!=0)
				{
					GlobalTools.popMessage(this,"Evaluation ID is Existing..");
					return;
				}
				//SMEDEV2
				conncc.QueryString="select Evaluation_ID from PENDING_SALESCOM_RFEvaluation ";
				conncc.ExecuteQuery();
				for(int i=0; i<conncc.GetRowCount(); i++)
				{
					if(conncc.GetFieldValue(i,0).ToString().Trim() == TXT_ID.Text.Trim())
					{
				...
				conncc.QueryString="PARAM_SALESCOM_RFEvaluation_MAKER '1','"+ 
					TXT_ID.Text +"','"+ TXT_EVAL_DESC.Text +"','"+seq_id+"','1'";
				conncc.ExecuteQuery();
				BlankEntry();
				ViewPendingData();
			}
			if (LBL_JENIS.Text=="edit" && LBL_ID.Text!="") //edit
			{
				...
				{
					//SALESMANDIRI
					conncc.QueryString="PARAM_SALESCOM_RFEvaluation_MAKER '"+LBL_SAVE_MODE.Text+"','"+ 
						TXT_ID.Text +"','"+ TXT_EVAL_DESC.Text +"','"+LBL_SEQ_ID.Text+"','1'";
					conncc.ExecuteQuery();
					BlankEntry();
					ViewPendingData();
				}
				...
				{
					conncc.QueryString="select * from PENDING_SALESCOM_RFEvaluation where Evaluation_ID='"+TXT_ID.Text+"'";
					conncc.ExecuteQuery();
					...
					conncc.QueryString="PARAM_SALESCOM_RFEvaluation_MAKER '0','"+ 
						TXT_ID.Text +"','"+ TXT_EVAL_DESC.Text +"','"+LBL_SEQ_ID.Text+"','1'";
					conncc.ExecuteQuery();
					BlankEntry();
					ViewPendingData();
