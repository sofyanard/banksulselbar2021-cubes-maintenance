...
{
	/// <summary>
	/// Summary description for ResignAppr.
	/// </summary>
	public partial class ResignAppr : System.Web.UI.Page
	{
		...
		{
			//SMEDEV2
			conncc.QueryString="select *, case when STATUS = '1' then 'Insert' when STATUS = '0' then 'Update' "+
					"else 'Delete' end STATUS1 from PENDING_SALESCOM_RFRESIGN";
			conncc.ExecuteQuery();
		...
		private void performRequestList(int row, int appr_sta)
		{
			...
			{	
				...
				string RE_CODE = DGR_APPR.Items[row].Cells[0].Text.Trim();
				string SEQ_ID = DGR_APPR.Items[row].Cells[6].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();
				//SMEDEV2
				conncc.QueryString="select * from PENDING_SALESCOM_RFRESIGN where RE_CODE='"+RE_CODE+"' and "+
					"SEQ_ID ='"+SEQ_ID+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string RE_DESC =conncc.GetFieldValue("RE_DESC");
						
				//Salesmandiri
				conncc.QueryString = "PARAM_SALESCOM_RFRESIGN_APPR '" + 
					appr_sta + "','"+ status +"','"+
					RE_CODE +"','"+ RE_DESC +"','"+ SEQ_ID +"','"+ userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch{}
		}
