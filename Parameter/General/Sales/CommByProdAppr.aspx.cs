...
{
	/// <summary>
	/// Summary description for CommByProdAppr.
	/// </summary>
	public partial class CommByProdAppr : System.Web.UI.Page
	{
		...
		private void performRequestList(int row, int appr_sta)
		{
			try
			{
				...
				string agentype=DGR_APPR.Items[row].Cells[8].Text.Trim();
				string level=DGR_APPR.Items[row].Cells[9].Text.Trim();
				string prod=DGR_APPR.Items[row].Cells[10].Text.Trim();
				string SEQ_ID = DGR_APPR.Items[row].Cells[11].Text.Trim();
				userid = Session["UserID"].ToString();
				groupid = Session["GroupID"].ToString();

				conncc.QueryString="select * from PENDING_SALESCOM_AGENT_COMMPRODUCT where AGENTYPE_ID='"+agentype+"' and LEVEL_CODE='"+level+"' and "+
					"PRODUCT_ID='"+prod+"' and SEQ_ID='"+SEQ_ID+"'";
				conncc.ExecuteQuery();
				string status=conncc.GetFieldValue("STATUS");
				string comm_value=GlobalTools.ConvertFloat(conncc.GetFieldValue("COMMISION_VALUE"));
				string active = conncc.GetFieldValue("ACTIVE");

				conncc.QueryString="PARAM_SALESCOM_AGENT_COMMPRODUCT_APPR '"+
					appr_sta +"','SMEDEV2','"+ status +"','"+
					agentype +"','"+ level +"','"+ prod +"','"+ comm_value +"','"+ active +"','"+SEQ_ID+"','"+
					userid +"','"+ groupid +"'";
				conncc.ExecuteQuery();
			}
			catch (Exception p)
