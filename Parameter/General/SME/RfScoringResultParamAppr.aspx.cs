...
namespace CuBES_Maintenance.Parameter.General.SME
{
	...
	/// </summary>
	public partial class RfScoringResultParamAppr : System.Web.UI.Page
	{
		...
		private void performRequest(int row)
		{
			try 
			{
				...
				conn.QueryString = "exec PARAM_GENERAL_RFSCORINGRESULTPARAM_APPR '" + a + "','" + b + "', '1', '" + Session["UserID"].ToString() + "'";
				...
				{
					conn.QueryString = "UPDATE rfscoringresultparam SET STW_CODE='"+b+"',STW_DESC='"+c+"',ACTIVE='1'  WHERE STW_TYPE='"+a+"' AND STW_CODE='"+b+"' ";
					conn.ExecuteNonQuery();
				}

				if (status.Equals("INSERT"))
				{
					conn.QueryString = "INSERT INTO rfscoringresultparam(STW_TYPE,STW_CODE,STW_DESC) VALUES ('"+a+"','"+b+"','"+c+"','1')";
					conn.ExecuteNonQuery();
				}
				if (status.Equals("DELETE"))
				{
					conn.QueryString = "DELETE rfscoringresultparam WHERE STW_TYPE='"+a+"' AND STW_CODE='"+b+"' ";
					conn.ExecuteNonQuery();
				}

				conn.QueryString = "DELETE pending_rfscoringresultparam WHERE STW_TYPE='"+a+"' AND STW_CODE='"+b+"' ";
				conn.ExecuteNonQuery();
				***/
