...
{
	/// <summary>
	/// Summary description for RFWatchlistItemAppr.
	/// </summary>
	public partial class RFWatchlistItemAppr : System.Web.UI.Page
	{
		...
		private void performRequest(int row)
		{
			try 
			{
				string bu = DGRequest.Items[row].Cells[0].Text.Trim();
				string id = DGRequest.Items[row].Cells[2].Text.Trim();
				executeApproval(bu, id, "1");
			} 
			catch (Exception e)
			{
		...
		private void executeApproval(string bu, string id, string approvalFlag) 
		{
			...
			if (approvalFlag == "1")	// approved
			{
				...
				/// 
				conn.QueryString = "SELECT * FROM PENDING_RFWATCHLIST_ITEM WHERE BUSSUNITID = '" + bu + "' AND WATCHID ='" + id + "'";
				conn.ExecuteQuery();
				pendingStatus	= conn.GetFieldValue("CH_STA");
				desc			= conn.GetFieldValue("WATCHDESC");
				conn.ClearData();

				/// audit perubahan parameter 
				/// 
				AuditTrail(bu, id, desc, pendingStatus);

				/// memeriksa data yang existing
				/// 
				conn.QueryString = "SELECT COUNT(*) JUMLAH FROM RFWATCHLIST_ITEM WHERE BUSSUNITID = '" + bu + "' AND WATCHID ='" + id + "'";
				conn.ExecuteQuery();
				...
				if (pendingStatus == "0" || pendingStatus == "1") 
				{
					...
					if (jumlah > 0) 
					{
						conn.QueryString = "UPDATE RFWATCHLIST_ITEM SET WATCHDESC = '" + desc + 
							"', ACTIVE = '"+LBL_ACTIVE.Text.Trim()+"' WHERE BUSSUNITID = '" + bu + "' AND WATCHID ='" + id + "'";
					...
					else 
					{
						...
						if(LBL_ACTIVE.Text.Trim() == "1")
							conn.QueryString = "INSERT INTO RFWATCHLIST_ITEM " +
								"(BUSSUNITID, WATCHID, WATCHDESC, ACTIVE) " +
								" VALUES ('" + bu + "', '" + id + "', '" + desc + "', '1')";
						else
							conn.QueryString = "INSERT INTO RFWATCHLIST_ITEM " +
								"(BUSSUNITID, WATCHID, WATCHDESC) " +
								" VALUES ('" + bu + "', '" + id + "', '" + desc + "')";
						conn.ExecuteNonQuery();
					}
				}
				else if (pendingStatus == "2")	// delete
				{
					...
					{
						if (LBL_ACTIVE.Text.Trim() == "1")
							conn.QueryString = "UPDATE RFWATCHLIST_ITEM SET ACTIVE = '0' WHERE BUSSUNITID = '" + bu + "' AND WATCHID ='" + id + "'";
						else
							conn.QueryString = "DELETE FROM RFWATCHLIST_ITEM WHERE BUSSUNITID = '" + bu + "' AND WATCHID ='" + id + "'";
						conn.ExecuteNonQuery();
					}
				}
			}
		...
		private void AuditTrail(string bu, string id, string desc, string pending_status)
		{
			conn.ClearData();
			string desc_old = "";
			string action	= "";
			...
			//Get Current Data
			conn.QueryString = "SELECT * FROM RFWATCHLIST_ITEM WHERE BUSSUNITID = '" + bu + "' AND WATCHID ='" + id + "'";
			conn.ExecuteQuery();
			if (conn.GetRowCount() > 0)
			{
				desc_old		= conn.GetFieldValue("WATCHDESC");
			}
			...
			if (action == "0")	// Update
			{
				if (desc != desc_old)
				{
					ExecSPAuditTrail(id,"WATCHDESC",desc_old, desc, action);
				}
			} 
			else if (action == "1")	// Insert
			{
				ExecSPAuditTrail(id,"WATCHID", "", id, action);
				ExecSPAuditTrail(id,"WATCHDESC", "", desc, action);
				ExecSPAuditTrail(id,"ACTIVE", "", "1", action);
			} 
			else if (action == "2") // Delete
			{
				ExecSPAuditTrail(id,"WATCHID", id, "", action);
				ExecSPAuditTrail(id,"WATCHDESC", desc_old, "", action);
				if (LBL_ACTIVE.Text.Trim() == "1")
