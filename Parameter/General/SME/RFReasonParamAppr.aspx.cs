	...
	public partial class RFReasonParamAppr : System.Web.UI.Page
	{
		...
		private void performRequest(int row)
		{
			try 
			{
				...
				string type = DGRequest.Items[row].Cells[2].Text.Trim();
				executeApproval(id, "1",type);
			} 
			catch {}
		}
		...
		private void deleteData(int row)
		{
			try 
			{
				string id = DGRequest.Items[row].Cells[0].Text.Trim();
				string type = DGRequest.Items[row].Cells[2].Text.Trim();
				executeApproval(id, "0", type);
			} 
			catch {}
		}

		private void executeApproval(string id, string approvalFlag, string type) 
		{
			conn.QueryString = "exec PARAM_GENERAL_RFREASON_APPR '" + id + "','" + type + "','"+ approvalFlag +"', '" + 
