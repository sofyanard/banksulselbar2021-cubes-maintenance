...
namespace CuBES_Maintenance.Parameter.General.SME
{
	...
	public partial class RFZipCodeCityParam : System.Web.UI.Page
	{
		...
		private void viewPendingData(string CITYID) 
		{
			...
			/// data di tabel pending tampil smua
//			conn.QueryString = "select * from VW_PARAM_GENERAL_PENDING_RFZIPCODECITY where CITYID = '"+ CITYID +"'";

			conn.QueryString = "select * from VW_PARAM_GENERAL_PENDING_RFZIPCODECITY ";
			conn.ExecuteQuery();
...
//			else
			...
			for (int i=0; i< DGExisting.Items.Count;i++)
			{
				...
				{		// active = 0
					/*
					LinkButton l_undel = (LinkButton) DGExisting.Items[i].FindControl("lnk_RfUnDelete");
...
//				}
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			{
				conn.QueryString = "select active from RFZIPCODECITY WHERE ZIPCODE ='" + TXT_ID.Text.Trim() + "'";
				conn.ExecuteQuery();
		...
		private void DGExisting_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				...
				case "delete":					
					 ...
					 coltypeid = e.Item.Cells[2].Text.Trim();
				...
				case "undelete":					
					 id = e.Item.Cells[0].Text.Trim();
					 coltypeid = e.Item.Cells[2].Text.Trim();
					executeMaker(id, e.Item.Cells[1].Text, coltypeid, "0");
					viewPendingData(DDL_COLTYPEID.SelectedValue);
					break;
