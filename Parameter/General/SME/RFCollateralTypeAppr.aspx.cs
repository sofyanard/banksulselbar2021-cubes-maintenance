...
namespace CuBES_Maintenance.Parameter.General.SME
{
	...
	/// </summary>
	public partial class RFCollateralTypeAppr : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "select COLTYPESEQ,COLTYPEID,COLTYPEDESC,COLLINKTABLE,RATINGCODE,COLLTYPECODESIBS,PENDINGSTATUS from PENDING_RFCOLLATERALTYPE ";
			conn.ExecuteQuery();
		...
		private void performRequest(int row)
		{
		    try
		    {
		        ...
		        conn.QueryString =  "exec PARAM_GENERAL_RFCOLLATERALTYPE_APPR '" + 
                                    a + "', '" +
                                    b + "', '" +
		                            "1', '" +
				...
				{
					conn.QueryString = "UPDATE RFCOLLATERALTYPE SET RATINGCODE='"+b+"'  WHERE COLTYPESEQ='"+a+"' ";
					conn.ExecuteNonQuery();

					conn.QueryString = "DELETE PENDING_RFCOLLATERALTYPE WHERE COLTYPESEQ='"+a+"' ";
					conn.ExecuteNonQuery();
				}	
				***/
		    }
		...
		private void deleteData(int row)
		{
			...
			{
				string a = DTG_APPR.Items[row].Cells[0].Text.Trim();
				//string d = DTG_APPR.Items[row].Cells[4].Text.Trim();
			    
				conn.QueryString = "exec PARAM_GENERAL_RFCOLLATERALTYPE_APPR '" + a + "', '0', '" + Session["UserID"].ToString() + "'";
				conn.ExecuteNonQuery();
			} 
			catch {}
		}
