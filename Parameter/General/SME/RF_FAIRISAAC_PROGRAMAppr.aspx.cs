...
{
	/// <summary>
	/// Summary description for RF_FAIRISAAC_PROGRAMAppr.
	/// </summary>
	public partial class RF_FAIRISAAC_PROGRAMAppr : System.Web.UI.Page
	{
		...
		private void performRequest(int row, string _status)
		{
			try 
			{
				...
				string _fi_id = DTG_APPR.Items[row].Cells[2].Text.Trim();

				conn.QueryString = "exec PARAM_GENERAL_RF_FAIRISSAC_PROGRAM_APPR '" + _progid + "', '" + _fi_id + "', '" + _status + "', '" + Session["UserID"].ToString() + "'";
