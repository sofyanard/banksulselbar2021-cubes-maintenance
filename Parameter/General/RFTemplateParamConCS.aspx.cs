...
{
	/// <summary>
	/// Summary description for RFTemplateParamConCS.
	/// </summary>
	public partial class RFTemplateParamConCS : System.Web.UI.Page
	{
		...
		private void viewPendingData() //SMEDEV2 (conn)
		{
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			executeMaker(TXT_ID.Text.Trim(), TXT_DESC.Text.Trim(), LBL_SAVEMODE.Text.Trim(),TXT_CD_SIBS.Text.Trim());
			viewPendingData();
			clearControls();
		...
		private void DGExisting_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				...
				case "delete":
					...
					string cd_sibs = e.Item.Cells[2].Text.Trim();
					executeMaker(id,desc,"2",cd_sibs);
					viewPendingData();
					break;
				default :
					break;
		...
		private void executeMaker(string id, string desc, string pendingStatus, string cd_sibs) 
		{
			...
			{				
				myQueryString = "UPDATE T" +Request.QueryString["tablename"]+ " SET " + getColumnDesc()+
					"= '" +desc+ "',CD_SIBS='"+cd_sibs+"', CH_STA = '" +pendingStatus+ "' WHERE "+getColumnKey()+ "= '"+id+"'";
				conn2.QueryString = myQueryString;
			}
			else 
			{
				myQueryString="INSERT INTO T" +Request.QueryString["tablename"]+
					"("+ getColumnKey() + "," + getColumnDesc() + ",CD_SIBS,CH_STA) VALUES ('"+id+"', '"+desc+"','"+cd_sibs+"','"+pendingStatus+"')";
				conn2.QueryString = myQueryString;
			}
