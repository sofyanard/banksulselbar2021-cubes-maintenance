...
{
	/// <summary>
	/// Summary description for TrackListParamAppr.
	/// </summary>
	public partial class TrackListParamAppr : System.Web.UI.Page
	{
		...
		private void performRequest(int row)
		{
			try 
			{
				...
				string TR_CODE = DGR_APPR.Items[row].Cells[0].Text.Trim();
				conn.QueryString="select * from TRFTRACKLST where TR_CODE='"+TR_CODE+"'";
				conn.ExecuteQuery();
				string status = conn.GetFieldValue("CH_STA");
				string TR_DESC = conn.GetFieldValue("TR_DESC");
				int TR_TARGET = int.Parse(conn.GetFieldValue("TR_TARGET"));
				int MAX_PROCESSTIME = int.Parse(conn.GetFieldValue("MAX_PROCESSTIME"));
				string SEND_STATUS = conn.GetFieldValue("SEND_STATUS");
				string REMARK = conn.GetFieldValue("REMARK");
				
				if (status.Equals("0"))
				{
					conn.QueryString = "EXEC PARAM_GENERAL_RFTRACKLST '3','"+TR_CODE+"','" +
						TR_DESC+ "', '" +TR_TARGET+ "', '" + MAX_PROCESSTIME +"','"+
						SEND_STATUS+ "', '" +REMARK+ "', '" + status+"','"+userid+"','"+groupid+"'" ;
					conn.ExecuteNonQuery();
				}

				if (status.Equals("1"))
				{
					conn.QueryString = "EXEC PARAM_GENERAL_RFTRACKLST '4','"+TR_CODE+"','" +
						TR_DESC+ "', '" +TR_TARGET+ "', '" + MAX_PROCESSTIME +"','"+
						SEND_STATUS+ "', '" +REMARK+ "', '" + status+"','"+userid+"','"+groupid+"'" ;
					conn.ExecuteNonQuery();
				}
				if (status.Equals("2"))
				{
					conn.QueryString = "EXEC PARAM_GENERAL_RFTRACKLST '5','"+TR_CODE+"','" +
						TR_DESC+ "', '" +TR_TARGET+ "', '" + MAX_PROCESSTIME +"','"+
						SEND_STATUS+ "', '" +REMARK+ "', '" + status+"','"+userid+"','"+groupid+"'" ;
					conn.ExecuteNonQuery();
				}
