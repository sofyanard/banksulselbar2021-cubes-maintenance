...
{
	/// <summary>
	/// Summary description for HomeFacParamAppr.
	/// </summary>
	public partial class HomeFacParamAppr : System.Web.UI.Page
	{
		...
		private void deleteData(int row)
		{
			try 
			{
				...
				conn.QueryString = "DELETE FROM TRFHOMFAC WHERE FC_CODE = '"+cd+"'";
				conn.ExecuteQuery();
		...
		private void performRequest(int row)
		{
			...
			conn.QueryString = "select * from RFHOMFAC where FC_CODE = '"+code+"'";
			conn.ExecuteQuery();
			string FC_CODE_OLD = conn.GetFieldValue("FC_CODE");
			string FC_DESC_OLD = conn.GetFieldValue("FC_DESC");
			...
			if (status.Equals("1"))
			{
				try
				{
					ExecSPAuditTrail(code,"FC_CODE","",code,"1");
					ExecSPAuditTrail(code,"FC_DESC","",desc,"1");
					ExecSPAuditTrail(code,"CD_SIBS","",sbc,"1");

					conn.QueryString = "INSERT INTO RFHOMFAC(FC_CODE, FC_DESC, CD_SIBS, ACTIVE)"+ 
			...
			if (status.Equals("2"))
			{
				try
				{
					if(code.Trim()!= FC_CODE_OLD.Trim())
					{
						ExecSPAuditTrail(code,"FC_CODE",FC_CODE_OLD,code,"0");
					}
					if(desc.Trim()!= FC_DESC_OLD.Trim())
					{
						ExecSPAuditTrail(code,"FC_DESC",FC_DESC_OLD,desc,"0");
					}
					...
					conn.QueryString = "UPDATE RFHOMFAC SET FC_DESC = "+GlobalTools.ConvertNull(desc)+", CD_SIBS = "+GlobalTools.ConvertNull(sbc)+" "+
						"where FC_CODE = '"+code+"'";
					conn.ExecuteQuery();
				}
				catch(Exception Ex)
			...
			if (status.Equals("3"))
			{
				try
				{
					ExecSPAuditTrail(code,"FC_CODE",code,"","2");
					ExecSPAuditTrail(code,"FC_DESC",desc,"","2");
					ExecSPAuditTrail(code,"CD_SIBS",sbc,"","2");

					conn.QueryString = "UPDATE RFHOMFAC SET ACTIVE='0' WHERE FC_CODE = '"+code+"'";
					conn.ExecuteQuery();
				}
				catch(Exception Ex)
				{
