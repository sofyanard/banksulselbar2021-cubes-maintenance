...
{
	/// <summary>
	/// Summary description for InsAutoValueParamAppr.
	/// </summary>
	public partial class InsAutoValueParamAppr : System.Web.UI.Page
	{
		...
		private void bindData1()
		{
			conn.QueryString="select AI.ASS_CODE, AT.ASS_DESC, AI.ASS_TENOR, AI.ASS_CASHPERCENT, AI.ASS_CREDITPERCENT ,AI.CH_STA "+
							 ",case when AI.CH_STA='0' then 'Update' when AI.CH_STA='1' then 'Insert' when AI.CH_STA='2' then 'Delete' else '' end CH_STA1 "+
							 "from TRFASSINTR AI "+
		...
		private void performRequest(int row)
		{
			try 
			{
				...
				string ASS_CODE = DGR_APPR.Items[row].Cells[8].Text.Trim();
				string ASS_TENOR = DGR_APPR.Items[row].Cells[1].Text.Trim();
				conn.QueryString="select * from TRFASSINTR where ASS_CODE='"+
					ASS_CODE+"' and ASS_TENOR='"+ASS_TENOR+"'";
				conn.ExecuteQuery();
				string status = conn.GetFieldValue("CH_STA");
				string ASS_CASHPERCENT = GlobalTools.ConvertFloat(conn.GetFieldValue("ASS_CASHPERCENT"));
				string ASS_CREDITPERCENT = GlobalTools.ConvertFloat(conn.GetFieldValue("ASS_CREDITPERCENT"));
				
				if (status.Equals("0"))
				{
					conn.QueryString = "EXEC PARAM_GENERAL_RFASSINTR '3','"+ASS_CODE+"','" +
						ASS_TENOR+ "', '" +ASS_CASHPERCENT+ "', '" + ASS_CREDITPERCENT +"','"+status+"','"+userid+"','"+groupid+"'" ;
					conn.ExecuteNonQuery();
				}

				if (status.Equals("1"))
				{
					conn.QueryString = "EXEC PARAM_GENERAL_RFASSINTR '4','"+ASS_CODE+"','" +
						ASS_TENOR+ "', '" +ASS_CASHPERCENT+ "', '" + ASS_CREDITPERCENT +"','"+status+"','"+userid+"','"+groupid+"'" ;
					conn.ExecuteNonQuery();
				}
				if (status.Equals("2"))
				{
					conn.QueryString = "EXEC PARAM_GENERAL_RFASSINTR '5','"+ASS_CODE+"','" +
						ASS_TENOR+ "', '" +ASS_CASHPERCENT+ "', '" + ASS_CREDITPERCENT +"','"+status+"','"+userid+"','"+groupid+"'" ;
					conn.ExecuteNonQuery();
				}

				conn.QueryString = "EXEC PARAM_GENERAL_RFASSINTR '2','"+ASS_CODE+"','" +
					ASS_TENOR+ "', '', '','','"+userid+"','"+groupid+"'" ;
				conn.ExecuteNonQuery();
