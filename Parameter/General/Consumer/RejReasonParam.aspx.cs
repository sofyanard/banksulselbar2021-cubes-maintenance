...
{
	/// <summary>
	/// Summary description for RejReasonParam.
	/// </summary>
	public partial class RejReasonParam : System.Web.UI.Page
	{
		...
		{
			conn.QueryString = "select * from RFREJECT where ACTIVE = '1' order by RJ_CODE";
			conn.ExecuteQuery();
		...
		private void CodeSeq(string mode)
		{
			...
			conn.QueryString = "select max(convert(int,isnull(RJ_CODE,0)))+ "+LBL_NB.Text+" as MAXSEQ from RFREJECT";
			conn.ExecuteQuery();
		...
		protected void BTN_SAVE_Click(object sender, System.EventArgs e)
		{
			...
			conn.QueryString = "SELECT RJ_CODE FROM TRFREJECT WHERE RJ_CODE = '"+TXT_CODE.Text+"'";
			conn.ExecuteQuery();
			...
			{
				conn.QueryString = "UPDATE TRFREJECT SET RJ_DESC = "+GlobalTools.ConvertNull(TXT_DESC.Text)+", CD_TMP = "+GlobalTools.ConvertNull(TXT_SIBS.Text)+" where RJ_CODE = '"+TXT_CODE.Text+"'";  					
				conn.ExecuteQuery();	
			...
			{
				conn.QueryString = "INSERT INTO TRFREJECT VALUES('"+TXT_CODE.Text+"',"+GlobalTools.ConvertNull(TXT_DESC.Text)+",NULL,'2',"+GlobalTools.ConvertNull(TXT_SIBS.Text)+")";
				conn.ExecuteQuery();
			...
			{
				conn.QueryString = "INSERT INTO TRFREJECT VALUES('"+TXT_CODE.Text+"',"+GlobalTools.ConvertNull(TXT_DESC.Text)+",NULL,'1',"+GlobalTools.ConvertNull(TXT_SIBS.Text)+")";
		...
		private void DG1_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				case "delete":
					...
					desc = e.Item.Cells[1].Text.Trim();
					cds = e.Item.Cells[2].Text.Trim();  

					conn.QueryString = "SELECT RJ_CODE FROM TRFREJECT WHERE RJ_CODE = '"+code+"'";
					conn.ExecuteQuery();
					...
					{
						conn.QueryString = "INSERT INTO TRFREJECT VALUES('"+code+"',"+GlobalTools.ConvertNull(desc)+",NULL,'3',"+GlobalTools.ConvertNull(cds)+")";
						conn.ExecuteQuery();
		...
		private void DG2_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			switch(((LinkButton)e.CommandSource).CommandName.ToLower())
			{
				case "delete":
					...
					conn.QueryString = "DELETE FROM TRFREJECT WHERE RJ_CODE = '"+code+"'";
					conn.ExecuteQuery();
					BindData2();
					break;
				case "edit":
