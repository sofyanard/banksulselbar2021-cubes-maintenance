...
{
	/// <summary>
	/// Summary description for CarModelParamAppr.
	/// </summary>
	public partial class CarModelParamAppr : System.Web.UI.Page
	{
		...
		{
			conn.QueryString ="select * from TMODEL";
			conn.ExecuteQuery();
		...
		private void performRequest(int row)
		{
			try 
			{
				...
				string id_model = DTG_APPR.Items[row].Cells[0].Text.Trim();
				string id_merek = DTG_APPR.Items[row].Cells[1].Text.Trim();
				string nm_model=DTG_APPR.Items[row].Cells[2].Text.Trim();
				string status=DTG_APPR.Items[row].Cells[3].Text.Trim();
							
				if (status.Equals("UPDATE"))
				{
					conn.QueryString ="EXEC PARAM_GENERAL_CARMODEL '3','" +
						id_model+ "', '" + id_merek + "', '" + 
						nm_model + "', '1' ,'"+userid+"','"+groupid+"' " ;
					conn.ExecuteNonQuery();
				}

				if (status.Equals("INSERT"))
				{
					conn.QueryString ="EXEC PARAM_GENERAL_CARMODEL '4','" +
						id_model+ "', '" + id_merek + "', '" + 
						nm_model + "', '1' ,'"+userid+"','"+groupid+"' " ;
					conn.ExecuteNonQuery();
				}
				...
				{
					conn.QueryString ="EXEC PARAM_GENERAL_CARMODEL '5','" +
						id_model+ "', '" + id_merek + "', '" + 
						nm_model + "', '1' ,'"+userid+"','"+groupid+"' " ;
					conn.ExecuteNonQuery();
				}
