...
{
	/// <summary>
	/// Summary description for BuildTypeParamAppr.
	/// </summary>
	public partial class BuildTypeParamAppr : System.Web.UI.Page
	{
		...
		private void fillDDL()
		{
			...
			DDL_MODULE.Items.Add(new ListItem("20 - Credit Card","20"));
		...
		private void BindData(string mode) 
		{
			if(mode == "01")
			{
				conn2.QueryString = "select * from PENDING_RFJENISBANGUNAN WHERE PENDINGSTATUS IN ('1','2','3')";
			}
			else if(mode == "20")
			{
				conn2.QueryString = "select * from PENDING_RFJENISBANGUNAN WHERE PENDINGSTATUS IN ('4','5','6')";
			}
			else if(mode == "40")
			{
				conn2.QueryString = "select * from PENDING_RFJENISBANGUNAN WHERE PENDINGSTATUS IN ('7','8','9')";
			}
		...
		private void deleteData(int row)
		{
			try 
			{
				...
				conn2.QueryString = "DELETE FROM PENDING_RFJENISBANGUNAN WHERE BANGUNANID = '"+cd+"'";
				conn2.ExecuteQuery();
		...
		{
			/* if table structure changes, must fix this!!! */
			if(DDL_MODULE.SelectedValue == "01")
			{
				LBL_TABNAME.Text = "RFJENISBANGUNAN";
				LBL_ID.Text = "BANGUNANID";
				LBL_DESC.Text = "BANGUNANDESC";
				LBL_SBC.Text = "";

			}
			else if(DDL_MODULE.SelectedValue == "20")
			{
				LBL_TABNAME.Text = "RFBUILDTYPE";
				LBL_ID.Text = "BUILDTYPEID";
				LBL_DESC.Text = "BUILDTYPEDESC";
		...
		private void performRequest(int row)
		{
			...
			if (status.Equals("4") || status.Equals("7"))
			{
				try
				{
					...
					conn.QueryString = "INSERT INTO "+LBL_TABNAME.Text+" ("+LBL_ID.Text+", "+LBL_DESC.Text+", "+LBL_SBC.Text+", ACTIVE) "+ 
			...
			if (status.Equals("5") || status.Equals("8"))
			{
				try
				{
					...
					conn.QueryString = "UPDATE "+LBL_TABNAME.Text+" SET "+LBL_DESC.Text+" = "+GlobalTools.ConvertNull(desc)+", "+
						"CD_SIBS = "+GlobalTools.ConvertNull(sbc)+" "+
			...
			if (status.Equals("3") || status.Equals("6") || status.Equals("9"))
			{
				try
				{
